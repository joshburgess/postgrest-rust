...
impl From<pg_wired::PgWireError> for ApiError {
    fn from(e: pg_wired::PgWireError) -> Self {
        Self::Pool(e.to_string())
    }
}

impl From<pg_pool::PoolError<pg_wired::PgWireError>> for ApiError {
...
pub fn map_wire_error(e: pg_wired::PgWireError) -> ApiError {
    match e {
        pg_wired::PgWireError::Pg(ref pg_err) => {
        ...
        other => ApiError::BadRequest(format!("pg-wired error: {other}")),
    }
}
