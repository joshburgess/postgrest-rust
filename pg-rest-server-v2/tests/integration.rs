...
//!
//! Run with:
//!   cargo test -p pg-rest-server-v2 --test integration
...
// ---------------------------------------------------------------------------
// Test harness — uses pg-wire only (no tokio-postgres)
// ---------------------------------------------------------------------------

async fn setup() -> axum::Router {
    let config = AppConfig {
        database: pg_rest_server_v2::config::DatabaseConfig {
        ...
        server: pg_rest_server_v2::config::ServerConfig::default(),
        jwt: pg_rest_server_v2::config::JwtConfig {
            secret: JWT_SECRET.to_string(),
        },
    };

    // Schema introspection via pg-wire (no tokio-postgres).
    let conn = pg_wire::WireConn::connect(
        "127.0.0.1:54322", "authenticator", "authenticator", "postgrest_test",
    ).await.unwrap();
    let mut pg = pg_wire::PgPipeline::new(conn);
    let cache = pg_schema_cache_v2::build_schema_cache(&mut pg, &config.database.schemas)
        .await
        .unwrap();
    drop(pg);
    ...
    pg_rest_server_v2::build_router(state)
}
