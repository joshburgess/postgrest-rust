...
pub fn cases(jwt: &str) -> Vec<TestCase> {
    vec![
        ...
        explain("EXPLAIN filter or", "/authors?or=(name.eq.Alice,name.eq.Bob)", jwt),
        ...
        explain("EXPLAIN with order nulls", "/books?order=published.asc.nullsfirst", jwt),

        // ==== EXPLAIN with select ====
        explain("EXPLAIN with select", "/authors?select=name", jwt),
        explain("EXPLAIN with select multi", "/books?select=title,pages", jwt),

        // ==== EXPLAIN with embedding ====
        explain("EXPLAIN embed O2M", "/authors?select=name,books(title)", jwt),
        explain("EXPLAIN embed M2O", "/books?select=title,authors(name)", jwt),
        explain("EXPLAIN embed M2M", "/books?select=title,tags(name)&id=eq.1", jwt),
        explain("EXPLAIN embed !inner", "/authors?select=name,books!inner(title)", jwt),
        explain("EXPLAIN embed spread", "/books?select=title,...authors(name)", jwt),

        // ==== EXPLAIN with complex queries ====
        explain("EXPLAIN complex", "/books?pages=gt.300&order=title.asc&limit=2", jwt),
        explain("EXPLAIN or+filter", "/items?or=(price.gt.20,quantity.gt.100)&active=eq.true", jwt),
