...
pub fn cases(jwt: &str) -> Vec<TestCase> {
    vec![
        ...
        // ==== JSONB contained-in ====
        g(
            "/entities?data=cd.{\"x\":1,\"tags\":[\"alpha\",\"beta\"],\"extra\":true}&order=id.asc",
            jwt,
        ),
        ...
        g(
            "/entities?select=name,data->>x&data=not.is.null&order=id.asc",
            jwt,
        ),
        g(
            "/items?select=name,metadata->>color&active=eq.true&order=id.asc",
            jwt,
        ),
