...
pub fn cases(jwt: &str) -> Vec<TestCase> {
    vec![
        ...
        // ==== SetOf functions ====
        post_json(
            "rpc/search_books Rust",
            "/rpc/search_books",
            json!({"query": "Rust"}),
            jwt,
        ),
        post_json(
            "rpc/search_books SQL",
            "/rpc/search_books",
            json!({"query": "SQL"}),
            jwt,
        ),
        post_json(
            "rpc/search_books no match",
            "/rpc/search_books",
            json!({"query": "Nonexistent"}),
            jwt,
        ),
        post_json(
            "rpc/search_books empty",
            "/rpc/search_books",
            json!({"query": ""}),
            jwt,
        ),
        ...
        post_json(
            "rpc/multi_defaults none",
            "/rpc/multi_defaults",
            json!({}),
            jwt,
        ),
        post_json(
            "rpc/multi_defaults partial a",
            "/rpc/multi_defaults",
            json!({"a": 10}),
            jwt,
        ),
        post_json(
            "rpc/multi_defaults partial ab",
            "/rpc/multi_defaults",
            json!({"a": 10, "b": 20}),
            jwt,
        ),
        post_json(
            "rpc/multi_defaults all",
            "/rpc/multi_defaults",
        ...
        post_json(
            "rpc/json_param nested",
            "/rpc/json_param",
            json!({"data": {"key": "nested", "extra": true}}),
            jwt,
        ),
        // ==== Volatile function ====
        ...
        post_json(
            "rpc/get_items_by_price",
            "/rpc/get_items_by_price",
            json!({"min_price": 10}),
            jwt,
        ),
        post_json(
            "rpc/get_items_by_price zero",
            "/rpc/get_items_by_price",
            json!({"min_price": 0}),
            jwt,
        ),
        post_json(
            "rpc/get_items_by_price high",
            "/rpc/get_items_by_price",
            json!({"min_price": 1000}),
            jwt,
        ),
        ...
        g(
            "/rpc/search_books?query=Rust&select=title&order=title.asc",
            jwt,
        ),
        g(
            "/rpc/authors_below?max_id=10&select=name&order=name.asc",
            jwt,
        ),
        ...
        post_json(
            "rpc/active_profiles default",
            "/rpc/active_profiles",
            json!({}),
            jwt,
        ),
        post_json(
            "rpc/active_profiles min80",
            "/rpc/active_profiles",
            json!({"min_score": 80}),
            jwt,
        ),
        post_json(
            "rpc/active_profiles min100",
            "/rpc/active_profiles",
            json!({"min_score": 100}),
            jwt,
        ),
        ...
        g(
            "/rpc/active_profiles?select=username,score&order=score.desc",
            jwt,
        ),
        g("/rpc/search_books?query=Rust&select=title", jwt),
        // ==== RPC singular response ====
        {
            let mut tc = post_json(
                "rpc/get_author singular",
                "/rpc/get_author",
                json!({"author_id": 1}),
                jwt,
            );
        ...
        g(
            "/rpc/active_profiles?select=username&order=username.asc",
            jwt,
        ),
        g(
            "/rpc/active_profiles?min_score=0&select=username,score&order=score.desc",
            jwt,
        ),
        post_json(
            "rpc/active_profiles min0",
            "/rpc/active_profiles",
            json!({"min_score": 0}),
            jwt,
        ),
        ...
        // ==== echo boundary cases ====
        post_json(
            "rpc/echo single char",
            "/rpc/echo",
            json!({"value": "x"}),
            jwt,
        ),
        post_json(
            "rpc/echo with newline",
            "/rpc/echo",
            json!({"value": "line1\nline2"}),
            jwt,
        ),
        ...
        post_json(
            "rpc/greet long name",
            "/rpc/greet",
            json!({"name": "a very long name indeed"}),
            jwt,
        ),
        ...
        post_json(
            "rpc/search_books Deep",
            "/rpc/search_books",
            json!({"query": "Deep"}),
            jwt,
        ),
        post_json(
            "rpc/search_books Basic",
            "/rpc/search_books",
            json!({"query": "Basic"}),
            jwt,
        ),
        g(
            "/rpc/search_books?query=Post&select=id,title&order=id.asc",
            jwt,
        ),
        // ==== get_items_by_price variations ====
        post_json(
            "rpc/get_items_by_price 5",
            "/rpc/get_items_by_price",
            json!({"min_price": 5}),
            jwt,
        ),
        g(
            "/rpc/get_items_by_price?min_price=20&select=name,price",
            jwt,
        ),
        // ==== count_by_status ====
        post_json(
            "rpc/count_by_status completed",
            "/rpc/count_by_status",
            json!({"s": "completed"}),
            jwt,
        ),
        post_json(
            "rpc/count_by_status pending",
            "/rpc/count_by_status",
            json!({"s": "pending"}),
            jwt,
        ),
        post_json(
            "rpc/count_by_status shipped",
            "/rpc/count_by_status",
            json!({"s": "shipped"}),
            jwt,
        ),
        post_json(
            "rpc/count_by_status none",
            "/rpc/count_by_status",
            json!({"s": "nonexistent"}),
            jwt,
        ),
        ...
        // ==== customer_orders ====
        post_json(
            "rpc/customer_orders Alice",
            "/rpc/customer_orders",
            json!({"cust": "Alice"}),
            jwt,
        ),
        post_json(
            "rpc/customer_orders Bob",
            "/rpc/customer_orders",
            json!({"cust": "Bob"}),
            jwt,
        ),
        post_json(
            "rpc/customer_orders nobody",
            "/rpc/customer_orders",
            json!({"cust": "Nobody"}),
            jwt,
        ),
        g(
            "/rpc/customer_orders?cust=Alice&select=customer,amount&order=amount.asc",
            jwt,
        ),
        g("/rpc/customer_orders?cust=Carol", jwt),
        // ==== concat_strings ====
        post_json(
            "rpc/concat_strings basic",
            "/rpc/concat_strings",
            json!({"a": "hello", "b": "world"}),
            jwt,
        ),
        post_json(
            "rpc/concat_strings sep",
            "/rpc/concat_strings",
            json!({"a": "foo", "b": "bar", "sep": "-"}),
            jwt,
        ),
        post_json(
            "rpc/concat_strings default sep",
            "/rpc/concat_strings",
            json!({"a": "x", "b": "y"}),
            jwt,
        ),
        ...
        post_json(
            "rpc/clamp in range",
            "/rpc/clamp",
            json!({"val": 5, "lo": 1, "hi": 10}),
            jwt,
        ),
        post_json(
            "rpc/clamp below",
            "/rpc/clamp",
            json!({"val": -5, "lo": 0, "hi": 100}),
            jwt,
        ),
        post_json(
            "rpc/clamp above",
            "/rpc/clamp",
            json!({"val": 999, "lo": 0, "hi": 100}),
            jwt,
        ),
        post_json(
            "rpc/clamp edge",
            "/rpc/clamp",
            json!({"val": 10, "lo": 10, "hi": 10}),
            jwt,
        ),
        ...
        // ==== sum_amounts ====
        post_json(
            "rpc/sum_amounts default",
            "/rpc/sum_amounts",
            json!({}),
            jwt,
        ),
        post_json(
            "rpc/sum_amounts min100",
            "/rpc/sum_amounts",
            json!({"min_amount": 100}),
            jwt,
        ),
        post_json(
            "rpc/sum_amounts min1000",
            "/rpc/sum_amounts",
            json!({"min_amount": 1000}),
            jwt,
        ),
        ...
        post_json(
            "rpc/concat_strings empty",
            "/rpc/concat_strings",
            json!({"a": "", "b": ""}),
            jwt,
        ),
        post_json(
            "rpc/concat_strings unicode",
            "/rpc/concat_strings",
            json!({"a": "café", "b": "naïve", "sep": " & "}),
            jwt,
        ),
        g("/rpc/concat_strings?a=hello&b=world&sep=%20", jwt),
        post_json(
            "rpc/clamp equal bounds",
            "/rpc/clamp",
            json!({"val": 5, "lo": 5, "hi": 5}),
            jwt,
        ),
        post_json(
            "rpc/clamp at lo",
            "/rpc/clamp",
            json!({"val": 0, "lo": 0, "hi": 10}),
            jwt,
        ),
        post_json(
            "rpc/clamp at hi",
            "/rpc/clamp",
            json!({"val": 10, "lo": 0, "hi": 10}),
            jwt,
        ),
        post_json(
            "rpc/count_by_status all",
            "/rpc/count_by_status",
            json!({"s": ""}),
            jwt,
        ),
        ...
        g(
            "/rpc/customer_orders?cust=Alice&select=amount&order=amount.desc",
            jwt,
        ),
        post_json(
            "rpc/sum_amounts 50",
