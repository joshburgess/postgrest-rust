...
All three backends share the same query/schema/HTTP layers. The pg-wired and resolute backends pass 1013/1013 of the PostgREST compatibility suite; the deadpool backend uses the same surface but has not been swept end-to-end as part of release validation. The choice comes down to dependency footprint and throughput goals:
